...
# why it is avoided. Each rule carries a sample line it must match, checked
...
FILES="$(git -C "$ROOT" ls-files -co --exclude-standard -- '*.sh' | grep -vxF "$SELF")"

FAILURES=0

# rule <description> <ERE> <sample line the ERE must match>
rule() {
    local desc="$1" regex="$2" sample="$3" f hits
    ...
    while IFS= read -r f; do
        [ -n "$f" ] || continue
        hits="$(grep -nE -- "$regex" "$ROOT/$f" | grep -vE '^[0-9]+:[[:space:]]*#' || true)"
        if [ -n "$hits" ]; then
            echo "FAIL $desc"
            printf '%s\n' "$hits" | sed "s|^|     $f:|"
            FAILURES=$((FAILURES + 1))
...
rule "sed -i (in-place flag differs between GNU and BSD sed; write a temp file and cat it back)" \
    'sed[[:space:]]+(-[A-Za-z]*i|--in-place)' 'sed -i "s/a/b/" f'
...
rule "bash 4 case modification (\${v,,} \${v^^}; use tr)" \
    '\$\{[A-Za-z_][A-Za-z0-9_]*(,,?|\^\^?)\}' 'x="${v,,}"'
